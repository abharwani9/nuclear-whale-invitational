...
export default function MediaGallery() {
  ...
  return (
    <div>
      <style>{`
        .media-tab-btn{padding:8px 18px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.08);border-radius:20px;color:rgba(255,255,255,0.45);font-family:inherit;font-size:13px;font-weight:600;cursor:pointer;transition:all 0.15s;white-space:nowrap;}
        ...
        .photo-thumb .overlay{position:absolute;inset:0;background:rgba(0,0,0,0.4);opacity:0;transition:opacity 0.2s;display:flex;align-items:center;justify-content:center;font-size:22px;}
        .photo-thumb:hover .overlay{opacity:1;}
        .lightbox-backdrop{position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:200;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:20px;}
        ...
        .audio-card{background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.08);border-radius:12px;padding:16px;margin-bottom:10px;}
        ...
        .doc-card{background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.08);border-radius:12px;padding:14px 16px;margin-bottom:10px;display:flex;align-items:center;gap:14px;cursor:pointer;transition:border-color 0.15s;}
        .doc-card:hover{border-color:rgba(255,255,255,0.2);}
        .dl-btn{padding:7px 14px;background:none;border:1px solid rgba(255,255,255,0.2);border-radius:8px;color:rgba(255,255,255,0.6);font-family:inherit;font-size:12px;font-weight:600;cursor:pointer;transition:all 0.15s;text-decoration:none;display:inline-block;}
        .dl-btn:hover{border-color:rgba(255,255,255,0.4);color:#fff;}
        .year-badge{display:inline-block;padding:2px 8px;border-radius:20px;font-size:11px;font-weight:700;background:rgba(255,255,255,0.07);color:rgba(255,255,255,0.45);margin-left:8px;}
      `}</style>
      ...
      {mediaTab === "photos" && (
        <div>
          {photos.length === 0
            ? <EmptyState icon="📸" text="No photos yet — admins can upload from the Admin Panel, and players can upload too!" />
            : (
              <>
                {/* Group by year */}
      ...
      {mediaTab === "audio" && (
        <div>
          {audio.length === 0
            ? <EmptyState icon="🎵" text="No audio yet — add theme songs or highlights from the Admin Panel" />
            : audio.map(a => (
              <div key={a.id} className="audio-card">
                <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 6 }}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    {a.year && <span className="year-badge">{a.year}</span>}
                  </div>
                  <a href={a.url} download={a.name} className="dl-btn">⬇ Download</a>
                </div>
      ...
      {mediaTab === "docs" && (
        <div>
          {docs.length === 0
            ? <EmptyState icon="📄" text="No documents yet — add scorecards, rules PDFs, or other files from the Admin Panel" />
            : docs.map(d => (
              <div key={d.id} className="doc-card" onClick={() => setPdfViewer({ url: d.url, name: d.name })}>
                <div style={{ fontSize: 32 }}>{d.name?.toLowerCase().endsWith(".pdf") ? "📋" : "📄"}</div>
                <div style={{ flex: 1 }}>
                  ...
                  {d.year && <span className="year-badge">{d.year}</span>}
                </div>
                <div style={{ display: "flex", gap: 8 }}>
                  <button className="dl-btn" onClick={e => { e.stopPropagation(); }}>
                    <a href={d.url} download={d.name} style={{ color: "inherit", textDecoration: "none" }}>⬇ Save</a>
                  </button>
...
// Players can upload their own photos via Cloudinary
export function PlayerUploadZone() {
  ...
  const upload = async () => {
    if (!file || !name) return alert("Please choose a photo and enter your name");
    setUploading(true);
    setProgress(0);
    try {
      ...
      alert("✅ Photo uploaded! It'll appear in the gallery shortly.");
    } catch (e) {
    ...
    <div style={{ marginTop: 24, background: "rgba(0,170,255,0.05)", border: "1px dashed rgba(0,170,255,0.25)", borderRadius: 12, padding: 18 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 8 }}>
        <div>
          ...
          <input style={inputStyle} value={name} onChange={e => setName(e.target.value)} placeholder="e.g. Mike T." />
        </div>
        <div>
          <div style={{ fontSize: 11, color: "rgba(255,255,255,0.4)", marginBottom: 4 }}>YEAR</div>
      ...
      <div style={{ display: "flex", gap: 8 }}>
        <button style={{ flex: 1, padding: "9px", background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.15)", borderRadius: 8, color: "rgba(255,255,255,0.7)", fontFamily: "inherit", fontSize: 13, cursor: "pointer" }}
          onClick={() => fileRef.current.click()}>
          {file ? "📷 Change Photo" : "📷 Choose Photo"}
        </button>
        {file && (
          <button style={{ flex: 2, padding: "9px", background: "linear-gradient(135deg,#0066cc,#00ccff)", border: "none", borderRadius: 8, color: "#fff", fontFamily: "inherit", fontSize: 13, fontWeight: 700, cursor: "pointer" }}
