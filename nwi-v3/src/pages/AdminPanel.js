...
const s = {
  ...
  btnGhost: { padding:"8px 16px", background:"none", border:"1px solid rgba(255,255,255,0.2)", borderRadius:8, color:"rgba(255,255,255,0.6)", fontFamily:"inherit", fontSize:12, fontWeight:600, cursor:"pointer" },
...
export default function AdminPanel({ authed, onAuth, onBack }) {
  ...
  const handleSeed = async () => {
    if (!window.confirm("Populate database with starter data? Only do this once on a fresh setup.")) return;
  ...
  if (!authed) return (
    <div style={{ minHeight:"100vh", background:"#07090e", display:"flex", alignItems:"center", justifyContent:"center", fontFamily:"'Barlow Condensed',sans-serif", padding:20 }}>
      ...
      <div style={{ ...s.card, width:"100%", maxWidth:360, textAlign:"center" }}>
        ...
        <input type="password" placeholder="Enter access code" value={code} onChange={e=>setCode(e.target.value)} onKeyDown={e=>e.key==="Enter"&&tryLogin()}
          style={{ ...s.input, marginBottom:12, textAlign:"center", letterSpacing:"0.12em" }} />
    ...
    <div style={{ minHeight:"100vh", background:"#07090e", color:"#e8edf3", fontFamily:"'Barlow Condensed',sans-serif" }}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0;}
        ...
        .sec-btn{flex-shrink:0;padding:8px 14px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.07);border-radius:8px;color:rgba(255,255,255,0.45);font-family:inherit;font-size:13px;font-weight:600;cursor:pointer;white-space:nowrap;}
      ...
      <div style={{ maxWidth:720, margin:"0 auto", padding:"20px 16px 60px" }}>
        {toast && <div style={{ marginBottom:14, padding:"10px 16px", borderRadius:8, background:toast.err?"rgba(220,30,30,0.15)":"rgba(74,222,128,0.12)", border:`1px solid ${toast.err?"rgba(220,30,30,0.4)":"rgba(74,222,128,0.3)"}`, fontSize:14, color:toast.err?"#ff5555":"#4ade80" }}>{toast.msg}</div>}
        {section==="players" && <PlayersSection players={players} showToast={showToast}/>}
        {section==="draft" && <DraftSection players={players} showToast={showToast}/>}
        {section==="rounds" && <RoundsSection rounds={rounds} players={players} competitions={competitions} showToast={showToast}/>}
...
function PlayersSection({ players, showToast }) {
  ...
  const save = async () => {
    ...
    try {
      if (editing) { await firestore.update("players", editing, {...form, handicap:Number(form.handicap)}); showToast("Updated!"); setEditing(null); }
  ...
  return (
    <div>
      ...
      <div style={s.card}>
        <div style={{ fontSize:14, fontWeight:700, marginBottom:14, color:editing?"#ff8c00":"#4ade80" }}>{editing?"✏️ Editing":"➕ Add Player"}</div>
        <div style={s.grid2}>
          ...
          <div><div style={s.label}>Photo URL (optional)</div><input style={s.input} value={form.photoURL} onChange={e=>setForm(f=>({...f,photoURL:e.target.value}))} placeholder="Paste image URL"/></div>
        </div>
        <div style={{ marginTop:10 }}><div style={s.label}>Bio (optional)</div><textarea rows={2} value={form.bio} onChange={e=>setForm(f=>({...f,bio:e.target.value}))} placeholder="Fun fact, previous wins, etc."/></div>
      ...
      {["nukes","whales"].map(team=>(
        <div key={team} style={{ marginBottom:20 }}>
          <div style={{ fontSize:14, fontWeight:700, color:team==="nukes"?"#ff4500":"#00aaff", letterSpacing:"0.08em", marginBottom:10, textTransform:"uppercase" }}>{team==="nukes"?"☢️":"🐋"} {team}</div>
          {players.filter(p=>p.team===team).map(p=>(
            <div key={p.id} style={{...s.card, padding:"12px 14px", display:"flex", alignItems:"center", gap:12}}>
              {p.photoURL ? <img src={p.photoURL} alt={p.name} style={{ width:36, height:36, borderRadius:"50%", objectFit:"cover" }}/> : <div style={{ width:36, height:36, borderRadius:"50%", background:"rgba(255,255,255,0.06)", display:"flex", alignItems:"center", justifyContent:"center", fontWeight:800 }}>{p.name?.[0]}</div>}
              <div style={{ flex:1 }}><div style={{ fontSize:15, fontWeight:700 }}>{p.name}</div><div style={{ fontSize:12, color:"rgba(255,255,255,0.4)" }}>HCP {p.handicap}</div></div>
              <div style={s.row}>
                <button style={s.btnGhost} onClick={()=>{setEditing(p.id);setForm({name:p.name,team:p.team,handicap:p.handicap,bio:p.bio||"",photoURL:p.photoURL||""});}}>Edit</button>
                <button style={s.btnDanger} onClick={async()=>{if(window.confirm("Delete?"))await firestore.delete("players",p.id);}}>✕</button>
              </div>
            </div>
          ))}
        </div>
...
// ── DRAFT ─────────────────────────────────────────────────────────────────────
function DraftSection({ players, showToast }) {
  const { data: drafts } = useCollection("drafts");
  const blankDraft = { year: new Date().getFullYear().toString(), notes: "" };
  const [form, setForm] = useState(blankDraft);
  const [editing, setEditing] = useState(null);
  const [assignments, setAssignments] = useState({});

  const currentDraft = editing ? drafts.find(d => d.id === editing) : null;

  const saveDraft = async () => {
    try {
      if (editing) {
        await firestore.update("drafts", editing, { year: Number(form.year), notes: form.notes, assignments });
        showToast("Draft saved!"); setEditing(null); setAssignments({});
      } else {
        const initAssignments = {};
        players.forEach(p => { initAssignments[p.name] = p.team; });
        const ref = await firestore.add("drafts", { year: Number(form.year), notes: form.notes, assignments: initAssignments, createdAt: new Date().toISOString() });
        showToast("Draft created!"); setEditing(ref.id); setAssignments(initAssignments);
      }
      setForm(blankDraft);
    } catch(e) { showToast("Error: "+e.message, true); }
  };
  ...
  const allPlayerNames = [...new Set([...players.map(p=>p.name), ...Object.keys(assignments)])];

  return (
    <div>
      <div style={s.sectionTitle}>🎲 Draft</div>
      <div style={{ fontSize:13, color:"rgba(255,255,255,0.4)", marginBottom:16, fontFamily:"'Barlow',sans-serif" }}>
        Assign players to teams for each tournament year. Teams change year to year.
      </div>

      <div style={s.card}>
        <div style={{ fontSize:14, fontWeight:700, marginBottom:14, color:editing?"#ff8c00":"#4ade80" }}>{editing?"✏️ Edit Draft":"➕ New Draft Year"}</div>
        <div style={s.grid2}>
          ...
          <div><div style={s.label}>Notes (optional)</div><input style={s.input} value={form.notes} onChange={e=>setForm(f=>({...f,notes:e.target.value}))} placeholder="e.g. snake draft, 10 picks"/></div>
        </div>

        {editing && (
          <div style={{ marginTop:16 }}>
            <div style={s.label}>Team Assignments</div>
            <div style={{ display:"flex", flexDirection:"column", gap:8, marginTop:8 }}>
              {allPlayerNames.map(name => (
                <div key={name} style={{ display:"flex", alignItems:"center", gap:12, padding:"10px 14px", background:"rgba(255,255,255,0.03)", borderRadius:8 }}>
                  <div style={{ flex:1, fontSize:14, fontWeight:600 }}>{name}</div>
                  <select style={{ ...s.select, width:"auto" }} value={assignments[name]||"nukes"} onChange={e=>setAssignments(a=>({...a,[name]:e.target.value}))}>
                    <option value="nukes">☢️ Nukes</option>
                    <option value="whales">🐋 Whales</option>
                  </select>
                </div>
              ))}
              {/* Add player not in current roster */}
              <AddPlayerToDraft onAdd={name => setAssignments(a=>({...a,[name]:"nukes"}))} />
            </div>
          </div>
        )}

        <div style={{...s.row, marginTop:14}}>
          <button style={s.btnFire} onClick={saveDraft}>{editing?"Save Draft":"Create Draft"}</button>
          {editing && <button style={s.btnGhost} onClick={()=>{setEditing(null);setAssignments({});setForm(blankDraft);}}>Cancel</button>}
        </div>
      </div>

      {/* Past drafts */}
      {[...drafts].sort((a,b)=>b.year-a.year).map(d=>(
        <div key={d.id} style={{...s.card, padding:"14px 16px"}}>
          <div style={{ display:"flex", alignItems:"center", gap:12 }}>
            <div style={{ fontSize:22, fontWeight:900, color:"rgba(255,255,255,0.2)" }}>{d.year}</div>
            <div style={{ flex:1 }}>
              <div style={{ fontSize:14, fontWeight:700 }}>{d.year} Draft</div>
              {d.notes && <div style={{ fontSize:12, color:"rgba(255,255,255,0.4)" }}>{d.notes}</div>}
              <div style={{ display:"flex", gap:8, marginTop:6, flexWrap:"wrap" }}>
                {Object.entries(d.assignments||{}).map(([name,team])=>(
                  <span key={name} style={{ fontSize:11, padding:"2px 8px", borderRadius:20, background:team==="nukes"?"rgba(255,69,0,0.15)":"rgba(0,170,255,0.15)", color:team==="nukes"?"#ff4500":"#00aaff" }}>{team==="nukes"?"☢️":"🐋"} {name}</span>
                ))}
              </div>
            </div>
            <div style={s.row}>
...
function AddPlayerToDraft({ onAdd }) {
  const [name, setName] = useState("");
  return (
    <div style={{ display:"flex", gap:8, marginTop:4 }}>
      <input style={{ flex:1, background:"rgba(255,255,255,0.06)", border:"1px solid rgba(255,255,255,0.1)", borderRadius:8, color:"#e8edf3", fontFamily:"inherit", fontSize:13, padding:"8px 10px", outline:"none" }} value={name} onChange={e=>setName(e.target.value)} placeholder="Add player not in current roster..." />
      <button style={{ padding:"8px 14px", background:"rgba(255,255,255,0.07)", border:"1px solid rgba(255,255,255,0.15)", borderRadius:8, color:"rgba(255,255,255,0.6)", fontFamily:"inherit", fontSize:12, cursor:"pointer" }}
...
// ── ROUNDS ────────────────────────────────────────────────────────────────────
function RoundsSection({ rounds, players, competitions, showToast }) {
  ...
  const saveRound = async () => {
    ...
    try {
      ...
      if (editingRound) { await firestore.update("rounds",editingRound,data); showToast("Round updated!"); setEditingRound(null); }
  ...
  const addMatchup = async (round) => {
    const newM = { nukes:["",""], whales:["",""], winner:null };
    await firestore.update("rounds",round.id,{matchups:[...(round.matchups||[]),newM]});
  };
    ...
    <div>
      ...
      <div style={s.card}>
        ...
        <div style={{ marginTop:10 }}>
          <div style={s.label}>Competition Name (optional)</div>
      ...
      {rounds.map(round=>(
        <div key={round.id} style={{...s.card, borderColor:"rgba(255,200,0,0.15)"}}>
          <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:14 }}>
            <div style={{ flex:1 }}>
              <div style={{ fontSize:17, fontWeight:800 }}>{round.name} <span style={{ fontSize:12, color:"rgba(255,255,255,0.3)" }}>{round.day}</span></div>
            ...
            <button style={s.btnDanger} onClick={async()=>{if(window.confirm("Delete round?"))await firestore.delete("rounds",round.id);}}>✕</button>
          </div>
            ...
            <div key={mi} style={{ background:"rgba(255,255,255,0.03)", border:"1px solid rgba(255,255,255,0.07)", borderRadius:10, padding:"12px 14px", marginBottom:10 }}>
              <div style={s.grid2}>
                <div>
                  ...
                  {[0,1].map(idx=>(
                    <select key={idx} style={{...s.select,marginBottom:6}} value={(m.nukes||[])[idx]||""} onChange={e=>updateMatchupPlayer(round,mi,"nukes",idx,e.target.value)}>
                ...
                <div>
                  ...
                  {[0,1].map(idx=>(
                    <select key={idx} style={{...s.select,marginBottom:6}} value={(m.whales||[])[idx]||""} onChange={e=>updateMatchupPlayer(round,mi,"whales",idx,e.target.value)}>
                      <option value="">— Select player —</option>
    ...
    <div>
      ...
      <div style={s.card}>
        ...
        <div style={s.grid2}>
          ...
          <div><div style={s.label}>Course Name (optional)</div><input style={s.input} value={form.course} onChange={e=>setForm(f=>({...f,course:e.target.value}))} placeholder="e.g. Pebble Beach"/></div>
        </div>
          ...
          <div key={day} style={{ marginBottom:16 }}>
            ...
            {items.map(item=>(
              <div key={item.id} style={{...s.card,padding:"11px 14px",display:"flex",alignItems:"center",gap:10}}>
                ...
                <div style={{ flex:1 }}>
                  <div style={{ fontSize:14 }}>{item.event}</div>
                  {item.course&&<div style={{ fontSize:11, color:"rgba(255,255,255,0.4)" }}>📍 {item.course}</div>}
                </div>
  ...
  return (
    <div>
      ...
      <div style={s.card}>
        ...
        <div style={s.grid2}>
          <div style={{ marginTop:10 }}><div style={s.label}>Current Leader / Winner</div><input style={s.input} value={form.winner} onChange={e=>setForm(f=>({...f,winner:e.target.value}))} placeholder="Player name"/></div>
      ...
      {competitions.map(c=>(
        <div key={c.id} style={{...s.card,padding:"13px 16px",display:"flex",alignItems:"center",gap:12}}>
          ...
          <div style={{ flex:1 }}><div style={{ fontSize:15, fontWeight:700 }}>{c.name}</div><div style={{ fontSize:12, color:"rgba(255,255,255,0.4)" }}>{c.winner?`Leader: ${c.winner}`:"No result yet"}</div></div>
    ...
    <div>
        ...
        <div style={s.grid2}>
          ...
          <div><div style={s.label}>Winner</div>
            <select style={s.select} value={form.winner} onChange={e=>setForm(f=>({...f,winner:e.target.value}))}>
              <option value="THE NUKES">☢️ THE NUKES</option><option value="THE WHALES">🐋 THE WHALES</option>
            </select>
          </div>
      ...
      {[...history].sort((a,b)=>b.year-a.year).map(h=>(
        <div key={h.id} style={{...s.card}}>
          <div style={{ display:"flex", alignItems:"center", gap:12, padding:"0 0 2px" }}>
            <div style={{ fontSize:22, fontWeight:900, color:"rgba(255,255,255,0.15)", minWidth:50 }}>{h.year}</div>
            <div style={{ flex:1 }}>
            ...
            <button style={s.btnGhost} onClick={()=>setExpandedYear(expandedYear===h.id?null:h.id)}>{expandedYear===h.id?"Collapse ▲":"Details ▼"}</button>
            ...
            <button style={s.btnDanger} onClick={async()=>{if(window.confirm("Delete year?"))await firestore.delete("history",h.id);}}>✕</button>
          </div>
...
function MatchesEditor({ year, showToast }) {
  ...
  const removeMatch = async (mi) => {
    const matches=(year.matches||[]).filter((_,i)=>i!==mi);
    await firestore.update("history",year.id,{matches});
  };

  const updateWinner = async (mi, winner) => {
    const matches=(year.matches||[]).map((m,i)=>i===mi?{...m,winner}:m);
    await firestore.update("history",year.id,{matches});
  };

  return (
    <div style={{ marginBottom:20 }}>
      <div style={{ fontSize:13, fontWeight:700, color:"rgba(255,255,255,0.5)", letterSpacing:"0.08em", textTransform:"uppercase", marginBottom:12 }}>Matchup Results</div>
      <div style={{ background:"rgba(255,255,255,0.03)", border:"1px solid rgba(255,255,255,0.07)", borderRadius:10, padding:14, marginBottom:10 }}>
        <div style={{ fontSize:12, fontWeight:700, color:"#4ade80", marginBottom:10 }}>+ Add Match</div>
        <div style={s.grid2}>
          <div>
            <div style={{ fontSize:11, color:"#ff4500", marginBottom:4 }}>☢️ NUKES</div>
            {[0,1].map(i=><input key={i} style={{...s.input,marginBottom:6}} value={form.nukes[i]||""} onChange={e=>{const n=[...form.nukes];n[i]=e.target.value;setForm(f=>({...f,nukes:n}));}} placeholder={`Nuke player ${i+1}`}/>)}
          </div>
          <div>
            <div style={{ fontSize:11, color:"#00aaff", marginBottom:4 }}>🐋 WHALES</div>
            {[0,1].map(i=><input key={i} style={{...s.input,marginBottom:6}} value={form.whales[i]||""} onChange={e=>{const w=[...form.whales];w[i]=e.target.value;setForm(f=>({...f,whales:w}));}} placeholder={`Whale player ${i+1}`}/>)}
          </div>
          <div><div style={s.label}>Round Name</div><input style={s.input} value={form.roundName} onChange={e=>setForm(f=>({...f,roundName:e.target.value}))} placeholder="e.g. Round 1"/></div>
        ...
        <div style={{ display:"flex", gap:6, marginTop:10, flexWrap:"wrap" }}>
          ...
          {["nukes","tie","whales"].map(w=>(
            <button key={w} onClick={()=>setForm(f=>({...f,winner:w}))}
              style={{ padding:"4px 12px", borderRadius:8, border:`1px solid ${form.winner===w?(w==="nukes"?"#ff4500":w==="whales"?"#00aaff":"#ffd700"):"rgba(255,255,255,0.12)"}`, background:form.winner===w?"rgba(255,255,255,0.1)":"none", color:form.winner===w?"#fff":"rgba(255,255,255,0.4)", fontFamily:"inherit", fontSize:12, cursor:"pointer" }}>
              {w==="nukes"?"☢️ Nukes":w==="whales"?"🐋 Whales":"🤝 Tie"}
            </button>
          ))}
        </div>
        <button style={{...s.btnFire,marginTop:12,fontSize:12}} onClick={addMatch}>+ Add Match</button>
      </div>

      {(year.matches||[]).map((m,mi)=>(
        <div key={mi} style={{ background:"rgba(255,255,255,0.02)", border:"1px solid rgba(255,255,255,0.06)", borderRadius:8, padding:"10px 12px", marginBottom:8 }}>
          <div style={{ display:"flex", alignItems:"center", gap:10 }}>
            <div style={{ flex:1, fontSize:13 }}>
              <span style={{ color:m.winner==="nukes"?"#ff4500":"rgba(255,255,255,0.5)" }}>☢️ {(m.nukes||[]).join(" & ")}</span>
              <span style={{ color:"rgba(255,255,255,0.2)", margin:"0 8px" }}>vs</span>
              <span style={{ color:m.winner==="whales"?"#00aaff":"rgba(255,255,255,0.5)" }}>{(m.whales||[]).join(" & ")} 🐋</span>
              {m.winner==="tie"&&<span style={{ color:"#ffd700", marginLeft:8 }}>TIE</span>}
            </div>
            <div style={{ fontSize:11, color:"rgba(255,255,255,0.3)" }}>{m.roundName} · {m.pointsWorth}pts</div>
            <button style={s.btnDanger} onClick={()=>removeMatch(mi)}>✕</button>
          </div>
          <div style={{ display:"flex", gap:6, marginTop:8 }}>
            {["nukes","tie","whales"].map(w=>(
              <button key={w} onClick={()=>updateWinner(mi,w)}
                style={{ padding:"3px 10px", borderRadius:6, border:`1px solid ${m.winner===w?(w==="nukes"?"#ff4500":w==="whales"?"#00aaff":"#ffd700"):"rgba(255,255,255,0.1)"}`, background:"none", color:m.winner===w?"#fff":"rgba(255,255,255,0.4)", fontFamily:"inherit", fontSize:11, cursor:"pointer" }}>
                {w==="nukes"?"☢️":w==="whales"?"🐋":"🤝"}
              </button>
            ))}
          </div>
...
function SuperlativesEditor({ year, showToast }) {
  ...
  const add = async () => {
    ...
    const superlatives=[...(year.superlatives||[]),{title,player}];
    await firestore.update("history",year.id,{superlatives});
    setTitle(""); setPlayer(""); showToast("Superlative added!");
  };

  const remove = async (si) => {
    const superlatives=(year.superlatives||[]).filter((_,i)=>i!==si);
    await firestore.update("history",year.id,{superlatives});
  };

  return (
    <div>
      <div style={{ fontSize:13, fontWeight:700, color:"rgba(255,255,255,0.5)", letterSpacing:"0.08em", textTransform:"uppercase", marginBottom:12 }}>Superlatives / Awards</div>
      <div style={{ display:"flex", gap:8, marginBottom:10 }}>
        <input style={{...s.input,flex:1}} value={title} onChange={e=>setTitle(e.target.value)} placeholder="Award name (e.g. MVP, Most Improved)"/>
        <input style={{...s.input,flex:1}} value={player} onChange={e=>setPlayer(e.target.value)} placeholder="Player name"/>
        <button style={s.btnFire} onClick={add}>+ Add</button>
      </div>
      <div style={{ display:"flex", flexWrap:"wrap", gap:8 }}>
        {(year.superlatives||[]).map((sup,si)=>(
          <div key={si} style={{ display:"flex", alignItems:"center", gap:6, padding:"4px 10px 4px 12px", background:"rgba(255,255,255,0.06)", border:"1px solid rgba(255,255,255,0.1)", borderRadius:20 }}>
            <span style={{ fontSize:12 }}>🏅 <strong>{sup.title}:</strong> {sup.player}</span>
            <button onClick={()=>remove(si)} style={{ background:"none", border:"none", color:"rgba(255,80,80,0.6)", cursor:"pointer", fontSize:13, padding:"0 0 0 4px" }}>✕</button>
          </div>
        ))}
      </div>
    </div>
...
function SettingsSection({ meta, showToast }) {
  ...
  const save = async () => {
    try { await firestore.set("meta","tournament",{...form,year:Number(form.year)}); showToast("Settings saved!"); }
    catch(e) { showToast(e.message,true); }
  };
  ...
  return (
    <div>
      ...
      <div style={s.card}>
        ...
        <div style={{ marginTop:16, padding:"12px 14px", background:"rgba(255,255,255,0.03)", border:"1px solid rgba(255,255,255,0.06)", borderRadius:8 }}>
          <div style={s.label}>App Password (for all users)</div>
          <div style={{ fontSize:13, color:"rgba(255,255,255,0.5)", fontFamily:"'Barlow',sans-serif" }}>Current: <strong style={{ color:"#ff4500" }}>nwi2026</strong> — to change, ask Claude to update the APP_PASSWORD in PublicApp.js</div>
        </div>
        <div style={{ marginTop:16, padding:"12px 14px", background:"rgba(255,255,255,0.03)", border:"1px solid rgba(255,255,255,0.06)", borderRadius:8 }}>
          <div style={s.label}>Admin Codes</div>
          <div style={{ fontSize:13, color:"rgba(255,255,255,0.5)", fontFamily:"'Barlow',sans-serif" }}>
            <strong style={{ color:"#ff4500" }}>nuke2026</strong> · <strong style={{ color:"#00aaff" }}>whale2026</strong> · <strong style={{ color:"#ffd700" }}>admin2026</strong>
          </div>
        </div>
